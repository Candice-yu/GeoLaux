...
client = OpenAI(
    ...
    api_key='sk-0BWy86Bq5t00lbDoTaYP7lg5IcZpjqd1BBKafK2QpxT2GPja',# evaluate_answer
    timeout=120
)
...
def create_completion(model: str,toevaluate_model, path_toevaluate, savefile_path: str):
    ...
    with tqdm(total=len(data), desc="Progress of topic processing") as pbar:
        for key in data:  
            ...
            reference_solution= "参考求解过程："+reference_solution
            student_answer=data[key][f"{toevaluate_model}_answer"]
            # if_auxiliary=data[key][f"{toevaluate_model}_answer"]
            # student_answer.pop("if_auxiliary")
            student_answer="学生解答："+str(student_answer)
            ...
            text = text+problem_text+reference_solution+student_answer  #complete prompt
            print(text)
            ...
            if response_correct(response):
                ...
                if(is_valid_json(cleaned_text)):
                    ...
                    data[key][f"{model}_evaluate"] = result_json
                else:
                    #Save formatted strings for easy post-processing
                    data[key][f"{model}_evaluate"] = cleaned_text
                    if(type!="proving"):
                ...
                result=response
                data[key][f"{model}_evaluate"] = f"response_error with {result}"
                with open(savefile_path, 'w', encoding='utf-8') as file:
                    json.dump(data, file, ensure_ascii=False, indent=4)
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Step-by-step score for different LMMs\' solution.')
...
if __name__ == "__main__":
    Calculation_prompt_path = 'prompts/score_cal_prompt.txt'
    Proof_prompt_path = 'prompts/score_pro_prompt.txt'
